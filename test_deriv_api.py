...
if __name__ == "__main__":
    try:
        if TOKEN in ("", "PEGA_AQUI_TU_TOKEN"):
            ...
            print("   - En tu PC: pega el token en la línea TOKEN de este archivo, o")
            print("     ejecuta: set DERIV_API_TOKEN=tu_token  (luego python test_deriv_api.py)")
            print("   - En GitHub Actions: crea el secreto DERIV_API_TOKEN "
        ...
        traceback.print_exc()
        print("   → ¿IP de EE.UU./VPN? Deriv bloquea esas redes (error 520).")
    finally:
        # En la nube (GitHub Actions CI) no hay consola: no pausar.
